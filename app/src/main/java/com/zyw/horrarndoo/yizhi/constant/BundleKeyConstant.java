package com.zyw.horrarndoo.yizhi.constant;

/**
 * Created by Horrarndoo on 2017/9/13.
 * <p>
 */

public class BundleKeyConstant {
    public static final String ARG_KEY_ZHIHU_DETAIL_ID = "arg_key_zhihu_detail_id   ";

    public static final String ARG_KEY_WANGYI_DETAIL_URL = "arg_key_wangyi_detail_id";
    public static final String ARG_KEY_WANGYI_DETAIL_TITLE = "arg_key_wangyi_detail_title";
    public static final String ARG_KEY_WANGYI_DETAIL_IMAGE_URL = "arg_key_wangyi_detail_url";
}
