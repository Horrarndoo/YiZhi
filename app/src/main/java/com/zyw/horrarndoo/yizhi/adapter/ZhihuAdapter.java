...
public class ZhihuAdapter extends BaseQuickAdapter<ZhihuDailyItemBean, BaseViewHolder> {
    ...
    protected void convert(final BaseViewHolder helper, ZhihuDailyItemBean item) {
        if (DBUtils.getDB(mContext).isRead(DBConfig.TABLE_ZHIHU, item.getId(), ItemState
                .STATE_IS_READ)) {
            helper.setTextColor(R.id.tv_daily_item_title, Color.GRAY);
        } else {
            if (SpUtils.getNightModel(mContext)) {
                helper.setTextColor(R.id.tv_daily_item_title, Color.WHITE);
            } else {
                helper.setTextColor(R.id.tv_daily_item_title, Color.BLACK);
            }
        }
